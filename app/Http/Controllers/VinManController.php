...
use App\DataTables\VinMansDataTable;
use App\DataTables\HistorialBusquedasManDataTable;
use App\DataTables\DetalleCampanaDataTable;
use Carbon\Carbon;
use Illuminate\Http\Request;
use Illuminate\Http\UploadedFile;
use App\Http\Requests;
use App\Http\Requests\CreateVinVolksRequest;
use App\Http\Requests\UpdateVinRequest;
use App\Repositories\VinMansRepository;
use Flash;
...
class VinManController extends AppBaseController

{

    /** @var  VinAdminrepository */

    private $vinAdminRepository;
    ...
    public function __construct(VinMansRepository $vinRepo)
    {
        //return response(self::$campañasNoAsiciaidaVin, 400);
        $this->vinAdminRepository = $vinRepo;
    }
    ...
    public function index(VinMansDataTable $vinDataTable)
    {
        return $vinDataTable->render('vins.indexMans');
    }
    ...
    public function historial_busquedas(HistorialBusquedasManDataTable $listavinsDataTable)
    {
    ...
    {
        $marcas = Marca::all();
        return view('vins.createmans')->with('marcas', $marcas);
    }
    ...
    public function store(CreateVinVolksRequest $request)
    {
        ...
        $vinObject = $this->vinAdminRepository->create($input);
        ...
        if ($request->vines) {
            ...
            $test = $spreadsheet->getActiveSheet()->getCell('B12');
            ...
            for ($row = 2; $row <= $highestRow; ++$row) {
                ...
                $siNumber = $worksheet->getCell("A". $row)->getValue();
                $pais = $worksheet->getCell("B" . $row)->getValue();
                $vin = $worksheet->getCell("C" . $row)->getValue();
                $startDate = $worksheet->getCell("D" . $row)->getValue();
                $endDate = $worksheet->getCell("E" . $row)->getValue();

               
                $data = array(
                    'vinvolksid' => $vinObject->id,
                    'siNumber' => $siNumber,
                    'pais' => $pais,
                    'vin' => $vin,
                    'startDate' => $startDate,
                    'endDate' => $endDate,
                    'campana' => $vinObject->campana,
                    'marca_id' => $vinObject->marca_id
                );

                DetalleVinMans::create($data);
                
            }
        }

        return redirect(route('vinsman.index'));
    }
    ...
    public function show($id)
    {
        ...
        $vin = $this->vinAdminRepository->findWithoutFail($id);
        if (empty($vin)) {
...

    {
        ...
        return view('vins.editman')->with(['vin' => $vin, 'marcas' => $marcas, 'lineas' => $lineas, 'modelos' => $modelos]);
...

    {
        ...
        $vin = $this->vinAdminRepository->update($data, $id);
...

    {
        ...
        $this->vinAdminRepository->delete($id);
    ...
    public function search(Request $request)
    {
        ...
        if (is_array($maracasRequest)) {
            echo "accpa";
            //$vin = DetalleCampana::join('VinVolks as vin', '//detallevinvolks.vinvolksid', 'vin.id')
                ...
                //});
                $vin = DetalleVinVolks::join('VinVolks as vin', 'detallevinvolks.vinvolksid', 'vin.id');
                //->where('detallevinvolks.startChasis', substr($request->vines, 0, 9))
            //->where('vin.estado', 1)

        } else {
            $vin = DetalleVinMans::join('vinmans as vin', 'detallevinmans.vinvolksid', 'vin.id')
                ->where('detallevinmans.vin', $request->vines)
                //->where('detallevinvolks.endChasis', substr($request->vines, 9));
                ;
        }
        ...
        if ($vin->count() > 0) {
            ...
            foreach ($vin as $value) {
                ...
                $vin_padre = VinMans::where('marca_id', $value->marca_id)->where('campana', $value->campana)->first();
    ...
    {
        //echo substr($vines, 0, 9);

        $vin = DetalleVinMans::join('vinmans as vin', 'detallevinmans.vinvolksid', 'vin.id')
            //->where('detallevinvolks.startChasis', substr($vines, 0, 9))
            //->where('vin.estado', 1)
            ->first();
