...
use App\DataTables\VinVolksDataTable;
use App\DataTables\HistorialBusquedasDataTable;
...
use App\Repositories\VinAdminRepository;
use Flash;
...
use App\Models\DetalleVinVolks;
...
class VinVolksController extends AppBaseController

{
    ...
    public function __construct(VinAdminRepository $vinRepo)
    {
    ...
    public function index(VinVolksDataTable $vinDataTable)
    {
        return $vinDataTable->render('vins.indexVolks');
    }
    ...
    {
        $marcas = Marca::all();
        return view('vins.createvolks')->with('marcas', $marcas);
    }
    ...
    public function store(CreateVinVolksRequest $request)
    {
        ...
        if ($request->vines) {
            ...
            for ($row = 12; $row <= $highestRow; ++$row) {
                ...
                $dn = $worksheet->getCell("B". $row)->getValue();
                $vehicle = $worksheet->getCell("C" . $row)->getValue();
                $startChasis = $worksheet->getCell("D" . $row)->getValue();
                $endChasis = $worksheet->getCell("E" . $row)->getValue();
        ...
        return redirect(route('vinsvokls.index'));
    }
...

    {
        ...
        return view('vins.editVolks')->with(['vin' => $vin, 'marcas' => $marcas, 'lineas' => $lineas, 'modelos' => $modelos]);
    ...
    public function search(Request $request)
    {
        ...
        if ($vin->count() > 0) {
            ...
            foreach ($vin as $value) {
                ...
                $vin_padre = VinVolks::where('marca_id', $value->marca_id)->where('campana', $value->campana)->first();
    ...
    public function historial_busquedas(HistorialBusquedasDataTable $listavinsDataTable)
    {
