 ...
 */
class VinVolks extends Model implements LogsActivityInterface
{
    //use SoftDeletes;
    use LogsActivity;

    public $table = 'VinVolks';
    ...
    static function getCampanasConsultadas($id)
    {
        ...
        $campanas_mas_consultadas = DetalleVinVolks::query()
            ->selectRaw('detallevinvolks.campana, count(distinct detallevinvolks.id) as total_afectados, 0 as total_atendidos, 
            (SELECT IFNULL(count(*), 0)
                    FROM (select vcon.campana, count(vcon.id) as cantidad_consultas from historial_busquedas as vcon inner join detallevinvolks as      
                    vcam on SUBSTRING(vcon.texto_busqueda, 0,9) = vcam.startChasis group by vcon.texto_busqueda, vcon.created_at) as v_vines_con
                    WHERE v_vines_con.campana = detallevinvolks.campana) as consultas_efectivas,
                    COUNT(historial_busquedas.texto_busqueda) as consultas_web')
            ->leftJoin('historial_busquedas', function($leftJoin){
            $leftJoin->on('detallevinvolks.campana', '=', 'historial_busquedas.campana');
            $leftJoin->on('detallevinvolks.startChasis', '=', \DB::raw('SUBSTRING(historial_busquedas.texto_busqueda, 0,9)'))
                ->where('historial_busquedas.estado', '=', 1 );
        })
        ->where('detallevinvolks.marca_id', $id)->groupBy('detallevinvolks.campana')
